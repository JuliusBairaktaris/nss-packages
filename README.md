...
[qosmio/nss-packages](https://github.com/qosmio/nss-packages); the
EDMA-data-plane adaptation is this tree's.
...
`qca-nss-clients` is trimmed to the four KernelPackages this stack uses — the NSS
qdisc, the ingress-shaping action, the PPPoE connection manager, and the bridge
manager (`kmod-qca-nss-drv-bridge-mgr`, wired LAN-bridge offload). The other
managers, their compatibility patches and init scripts are not carried.
...
main, carry the 5.15 → 6.12 kernel/toolchain compatibility patch queues, and
